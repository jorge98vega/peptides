...
def load_hist_data(histfile, col=2, n_last=25000):
    data = []
    with open(histfile) as f:
        for line in f:
            ...
            try:
                ...
                continue
    return np.array(data[-n_last:] if len(data) > n_last else data)
...
def main():
    ...
    parser.add_argument("--ylabel",     default="Free energy F(x) [kcal/mol]")
    ...
    parser.add_argument("--n-last",     type=int, default=25000, dest="n_last",
                        help="Use only the last N lines of the histogram file")
    ...
    for i, ((xi, Fi), folder) in enumerate(zip(datasets, args.folders)):
        ...
        label = folder if total == 0 else f"{folder} (offset={total:.2f})"
        ...
        if i == 0:
            ...
            ax.text(xi.min() + 0.45, ymin, "1", color=color, fontsize=12, va="center")
        elif i == 1:
            ymin = (Fi + total).min()
            ax.annotate("", xy=(xi.max() - 0.4, ymin), xytext=(xi.max() - 0.1, ymin),
                        arrowprops=dict(arrowstyle="->", color=color, lw=2))
            ax.text(xi.max() - 0.45, ymin, "2", color=color, fontsize=12, va="center")

    if args.histfile:
        values = load_hist_data(args.histfile, args.histcol, args.n_last)
        ax2 = ax.twinx()
        ax2.hist(values, bins=args.histbins, color="gray", alpha=0.3, density=True)
        ax2.set_ylabel("Probability density", color="gray")
